...
import com.baidu.duer.files.compat.removeFlagsCompat
...
fun Collection<Uri>.createSendStreamShareIntent(
...
fun ArrayList<Uri>.createSendFileIntent(mimeType: String, resolveInfo: ResolveInfo): Intent =
...
fun Uri.createOpenFileOtherApk(mimeType: String, packageName: String, className: String): Intent =
    Intent(Intent.ACTION_VIEW)
        .setComponent(ComponentName(packageName, className))
        .setDataAndType(this, mimeType)
        .addFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION)
        .addFlags(Intent.FLAG_GRANT_WRITE_URI_PERMISSION)
