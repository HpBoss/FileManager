...
import com.baidu.duer.files.app.BackgroundActivityStarter
import com.baidu.duer.files.app.application
import com.baidu.duer.files.app.mainExecutor
import com.baidu.duer.files.dialog.FileExceptionDialogFragment
import com.baidu.duer.files.file.*
import com.baidu.duer.files.filelist.OpenFileAsDialogActivity
import com.baidu.duer.files.filelist.OpenFileAsDialogFragment
...
import com.baidu.duer.files.navigation.getTargetPathForDuplicate
import com.baidu.duer.files.navigation.sendFileOperateProgress
import com.baidu.duer.files.provider.archive.archiveFile
import com.baidu.duer.files.provider.archive.archiver.ArchiveWriter
...
private fun FileJob.postTransferSizeNotification(
    @OperateType operateType: String = OperateType.NONE,
...
private fun FileJob.showActionDialog(
    ...
    runBlocking<ActionResult> {
        suspendCoroutine { continuation ->
            BackgroundActivityStarter.startActivity(FileJobExceptionDialogActivity::class.createIntent()
                .putArgs(
                    FileExceptionDialogFragment.Args(
                        title.toString(),
                        message.toString()
                    ) { action ->
                        continuation.resume(ActionResult(action, false))
                    }
...
class CopyFileJob(
    ...
    override fun run() {
        ...
        var target: Path = Paths.get(DEFAULT_PATH)
        for (source in sources) {
            target = if (source.parent == targetDirectory) {
                getTargetPathForDuplicate(source, source.isDirectory())
            } else {
                val tempTarget = targetDirectory.resolveForeign(getTargetFileName(source))
                getTargetPathForDuplicate(tempTarget, source.isDirectory())
            }
            if (isExtract) {
                LiveEventBus
                    .get(TargetFileName::class.java)
                    .post(TargetFileName(target.fileName.toString()))
            }
        ...
        sendFileOperateProgress(
            TaskProgressInfo(
                ...
                scanInfo.size,
                FileOperateTaskProgress.FINISH,
                if (isExtract) OperateType.UNZIP else OperateType.COPY
            )
        )
        if (!isExtract) application.showToast(getString(R.string.file_copy_success))
    }
    ...
    class DuplicateCountInfo(val countStart: Int, val countEnd: Int, val count: Int)
}
...
class MoveFileJob(private val sources: List<Path>, private val targetDirectory: Path) : FileJob() {
    ...
    override fun run() {
        ...
        for (source in sources) {
            if (source.parent == targetDirectory) continue
            val target = getTargetPathForDuplicate(
                targetDirectory.resolveForeign(source.fileName),
                source.isDirectory()
            )
            try {
        ...
        application.showToast(R.string.file_move_complete)
    }
    ...
    loop@ do {
        ...
        try {
            ...
            showActionDialog(
                getString(
                    type.getResourceId(
                        R.string.file_copy_exception_title,
                        R.string.file_unzip_exception_title,
                        R.string.file_move_exception_title
                    )
                ),
                getString(
                    type.getResourceId(
                        R.string.file_copy_exception_content,
                        R.string.file_unzip_exception_content,
                        R.string.file_copy_exception_content
                    ), getFileName(source)
                )
            )
